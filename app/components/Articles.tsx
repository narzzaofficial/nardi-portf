    ...
    <section className="py-16 md:py-28" style={{background:"#111114"}}>
      <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 md:mb-12 gap-4">
          ...
          <a href="#" className="flex items-center text-sm font-semibold shrink-0" style={{color:"rgba(238,238,242,0.5)"}}>
          ...
          {articles.map((a, i) => (
            <motion.article key={a.id || i} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5,delay:i*0.1}}
              ...
              <div className="h-36 md:h-44 rounded-2xl mb-4 relative overflow-hidden flex items-end p-4"
                style={{
                  background: a.coverImageUrl ? `url(${a.coverImageUrl}) center/cover` : "#16161a", 
                ...
                <span className="relative z-10 px-3 py-1 rounded-full text-xs font-semibold"
                  style={{background:"rgba(13,13,15,0.85)", border:"1px solid rgba(255,255,255,0.1)", color:"#eeeef2"}}>
                  {a.tag}
                </span>
              </div>
              ...
              <p className="text-sm leading-relaxed" style={{color:"rgba(238,238,242,0.45)"}}>{a.excerpt}</p>
            </motion.article>
          ))}
        </div>
      </div>
